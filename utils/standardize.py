...
def standardize(df):
    ...
    #正規化
    df.iloc[:,6:172:2] = df.iloc[:,6:172:2].apply(lambda x: (x - df["center_x"]) / df["face_rectangle_width"])
    df.iloc[:,7:172:2] = df.iloc[:,7:172:2] .apply(lambda x: (x - df["center_y"]) / df["face_rectangle_height"])
